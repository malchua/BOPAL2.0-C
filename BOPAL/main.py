...
outputFileName = 'BopalOutput.txt' #Name of output file
...
def main():
    ...
    if newickFileName == "tree2LeafNeighbour.dnd":
        outputFileName = sys.argv[2] + "/BopalNeighbourOutput.txt"
    else:
        outputFileName = sys.argv[2] + "/BopalOutput.txt"
    testFileName = sys.argv[2] + '/'
    
    print('Starting application...BOPAL')
    startTime = time.time()
    ...
    if newickFileName == "tree2LeafNeighbour.dnd":
        if len(root.clades) == 2:
            ...
            with open(testFileName + "bopalNeighbourRoot.txt", "w+") as f:
                f.write(rootGenome)
            neighbour.name = ''
            child.name = ''
    else:
        ...
        with open(testFileName + "bopalRoot.txt", "w+") as f:
            f.write(rootGenome)
