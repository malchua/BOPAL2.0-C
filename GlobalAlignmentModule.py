from MultiSeqAlignmentModule import findOrthologsByMultiSequenceAlignment
from MultiSeqAlignmentModule import splitByEvents
...
######################################################
def findOrthologsByGlobalAlignment(strain1, strain2, coverageTracker1, coverageTracker2, neighborEvents):
    events = []
    globalAlignmentCounter = 0
    strain1Copy = copy.deepcopy(strain1)
    ...
    events, coverageTracker1, coverageTracker2, globalAlignmentCounter, strain1, strain2 = scanGlobalAlignmentMatrixForOrthologs(alignmentMatrix, eventMatrix, coverageTracker1, coverageTracker2, strain1, strain2, neighborEvents)
...
######################################################
def scanGlobalAlignmentMatrixForOrthologs(globalAlignmentMatrix, eventMatrix, coverageTracker1, coverageTracker2, strain1, strain2, neighborEvents):
    events = []
    ...
    while currentScoreSelected >= 0:
        ...
        if bestEvent != None: #Good match was found so don't increment score incase more are found
            ...
            if neighborEvents != None:
                ...
                msaEvent = Event(0)
                msaEvent.setScore(1.0)
                msaEvent.setDistance(event.distance)
                msaEvent.setFragmentDetails1(copy.deepcopy(event.fragmentDetails1))
                msaEvent.setFragmentDetails2(copy.deepcopy(event.fragmentDetails2))
                msaEvent.setGenome1Name(strain1.name)
                msaEvent.setGenome2Name(strain2.name)
...
def reconstructOperonSequence(event, strain1, strain2):
    ...
    if len(operon1Gaps) == 0 and len(operon2Gaps) == 0:
        ...
        if event.technique == 'Multiple Sequence Alignment':
            operon1GapEvents = event.operon1GapEvents
            operon2GapEvents = event.operon2GapEvents
        ...
        while (i > 0) or (j > 0):
            ...
            if i > 0 and j > 0 and operon1GapIndexes[i-1] > operon2GapIndexes[j-1]:
                ...
                if event.technique == 'Multiple Sequence Alignment':
                    duplicationSizes, duplicationDetails, operon1Gaps[i-1], operon1GapPositions[i-1] = checkForMatchesWithinOperonsMSA(strain1.genomeFragments, event.fragmentDetails1, operon1Gaps[i-1], operon1GapPositions[i-1], operon1GapEvents[i-1])
                else:
                ...
                if event.technique == 'Multiple Sequence Alignment':
                    duplicationSizes, duplicationDetails, operon2Gaps[j-1], operon2GapPositions[j-1] = checkForMatchesWithinOperonsMSA(strain2.genomeFragments, event.fragmentDetails2, operon2Gaps[j-1], operon2GapPositions[j-1], operon2GapEvents[j-1])
                else:
...
# Description: Checks if a given gap exists in any of the other operons
######################################################
def checkForMatchesWithinOperonsMSA(genomeFragments, fragment, gap, positions, events):
    allDuplicationSizes = []
    allDuplicationDetails = ''

    newGaps, newPositions, newEvents = splitByEvents(gap, positions, events)
    remainingGaps = []
    remainingPositions = []
    ...
    for u in range(len(newGaps)-1, -1, -1):
        if events[u] == "dup":
            ...
            if len(gap) > 0:
                remainingGaps[0:0] = gap
                remainingPositions[0:0] = positions
        else:
            remainingGaps[0:0] = newGaps[u]
            remainingPositions[0:0] = newPositions[u]


    return allDuplicationSizes, allDuplicationDetails, remainingGaps, remainingPositions
